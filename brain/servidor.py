# Servidor do FLY ORGY: sobe o ENXAME (N cerebros em lote numa placa so), serve o site e transmite.
# Protocolo do quadro (binario): [uint32 tamanho do JSON][JSON][nuvem da mosca 0][nuvem 1]...[nuvem N-1],
# cada nuvem em delta-varint e com o tamanho em bytes listado em JSON['bytes']. A mosca em foco manda a
# nuvem inteira; as outras, uma amostra (o cerebrinho de cada uma na tela). O foco roda entre elas.
# Mensagens do cliente (texto JSON): {"estimulo": "sugar", "ms": 500, "mosca": 3}  (mosca -1 = todas)
import asyncio
import json
import os
import struct
...
from enxame import Enxame, ESTIMULOS_BLOQUEADOS  # noqa: E402
from relay_cliente import Uplink                 # noqa: E402

PORTA = int(os.environ.get('FLY_PORT', '8435'))
MOSCAS = int(os.environ.get('FLY_MOSCAS', '16'))               # tamanho do enxame (par: metade femea, metade macho)
...
PLASTICIDADE = os.environ.get('FLY_PLASTICIDADE', '0') != '0'   # off: as sinapses sao compartilhadas, aprender seria do enxame
MAX_IDX = int(os.environ.get('FLY_MAX_IDX', '3500'))            # teto de indices da mosca em foco
AMOSTRA_IDX = int(os.environ.get('FLY_AMOSTRA_IDX', '220'))     # indices por mosca fora do foco
...
def empacotar(cab, nuvens):
    """Uma nuvem por mosca, concatenadas; os tamanhos vao no cabecalho para o cliente fatiar."""
    partes = [delta_varint(n) for n in nuvens]
    cab['enc'] = 'dv'
    cab['bytes'] = [len(p) for p in partes]
    j = json.dumps(cab, separators=(',', ':')).encode('utf-8')
    return b''.join([struct.pack('<I', len(j)), j] + partes)


def mensagem_ola(app):
    cerebro = app['cerebro']
    return {
        'tipo': 'ola', 'quem': 'enxame', 'moscas': cerebro.B, 'pares': cerebro.B // 2,
        'n': cerebro.n, 'synapses': cerebro.n_sinapses, 'device': cerebro.device,
        ...
        'plasticity': cerebro.plasticidade, 'ambient_hz': 0.0,
    }
...
async def transmitir(app):
    ...
    while True:
        ...
        cab = {
            'tipo': 'quadro',
            'quem': 'enxame',
            'moscas': q['moscas'],                       # resumo de cada cerebro: spikes/s, estado, DN, regioes
            'foco': q['foco'],                           # de quem e a nuvem inteira neste quadro
            ...
            'ratio': round(10000.0 / pps, 1) if pps > 0 else None,
            'spikes': int(q['total']),
            ...
            'rot': round(((agora - app['estado']['t0']) * ROT_RAD_S) % 6.283185307, 4),
            'rot_speed': ROT_RAD_S,
        }
        ultimo_t = agora
    ...
    try:
        async for msg in ws:
            ...
            try:
                ...
                continue
            if 'estimulo' in m:
                ok = cerebro.estimular(m.get('mosca', -1), str(m['estimulo']), m.get('ms', 500), origem='site')
...
async def api_estimulo(request):
    ...
    ok = request.app['cerebro'].estimular(m.get('mosca', -1), str(m.get('estimulo', '')),
                                          m.get('ms', 500), origem='api')
...
async def api_versao(request):
    ...
    import hashlib
    h = hashlib.md5((SITE / 'publico.html').read_bytes() + (SITE / 'orgia-cliente.js').read_bytes()).hexdigest()[:12]
...
def main():
    reservar_cpu()
    cerebro = Enxame(
        MOSCAS, dir_dados=DADOS / 'flywire', dir_estado=DADOS / 'estado' / 'enxame',
        meta_parquet=DADOS / 'neuronios.parquet', plasticidade=PLASTICIDADE,
    )
    cerebro.iniciar()
    ...
    print(f'[servidor] http://localhost:{PORTA}  ({MOSCAS} moscas, plasticidade {PLASTICIDADE})',
          flush=True)
