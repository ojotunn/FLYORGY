...
MOSCAS = int(os.environ.get('FLY_MOSCAS', '16'))          # o enxame inteiro esta num servidor so
PARES = MOSCAS // 2
...
class Estocadas(threading.Thread):
    """Enquanto cruzam, CADA estocada bate nos dois cerebros DAQUELE par, no ritmo dele (ritmo_hz):
    JO (toque/vibracao, 209 neuronios a 300 Hz) nos dois = a pressao do encaixe; pC1 (receptividade)
    nela; a cada 4 estocadas, dopamina (PAM, 307) nos dois. Cada par tem seu proprio relogio, entao
    o que voce ve na tela bombando rapido e um par com libido alta de verdade."""
...
def estimular_par(nome, ms, p, quem='ambos'):
    if quem in ('ambos', 'ela'):
        estimular(nome, ms, 2 * p)
    if quem in ('ambos', 'ele'):
        estimular(nome, ms, 2 * p + 1)
...
class Libido:
    ...
    def ritmo_hz(self):
        return round(2.0 + 8.0 * self.v, 2)      # 2 a 10 Hz (o SEX FLY, com um casal so, ia de 1 a 6)
...
class Coro:
    """O enxame de pares. Uma compra acende os pares mais FRIOS (assim a onda percorre a sala em vez
    ...
    def compra(self, usd, p90):
        forca = min(1.0, usd / max(p90, 1e-9))
        quantos = max(1, min(self.n, int(round(1 + forca * (self.n - 1)))))
        ordem = sorted(range(self.n), key=lambda i: self.pares[i].v)[:quantos]
        for i in ordem:
            self.pares[i].compra(usd / quantos, max(p90 / max(1, quantos), 1e-9))
        return ordem

    def venda(self, usd, p50, p90):
        i = max(range(self.n), key=lambda k: self.pares[k].v)
        r = self.pares[i].venda(usd, p50, p90)
    ...
    def passo(self):
        return any([lib.passo() for lib in self.pares])

    def evento(self, extra=None):
        ev = {'classe': 'sexo',
              'pares': [{'i': i, 'libido': round(l.v, 3), 'estado': l.estado, 'ritmo': l.ritmo_hz()}
                        for i, l in enumerate(self.pares)],
              'transando': sum(1 for l in self.pares if l.estado == 'mating'),
              'libido_media': round(sum(l.v for l in self.pares) / max(1, self.n), 3),
              'ritmo_max': max(l.ritmo_hz() for l in self.pares) if self.pares else 0}
        if extra:
            ev['evento'] = extra
        return ev
...
def main():
    ...
    coro = Coro(PARES); Estocadas(coro).start()    # um relogio de estocada por par, batendo nos cerebros
    sent = None                            # feed proprio dos sentidos (SentidosChain) ou None
    sent_cfg = ''
    ficha_token = None                     # ficha do nosso token no indexador (preco, market cap, volume)
    prox_ficha = 0.0
